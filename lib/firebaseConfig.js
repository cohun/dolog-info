...
export async function deleteUserFromCompany(company, username) {
  const docRef = doc(db, 'companies', company);
  await updateDoc(docRef, {
    users: arrayRemove(username),
  });
  await deleteDoc(doc(db, `companies/${company}/${username}`, 'what'));
}
...
export async function delThing(company, id) {
  await deleteDoc(doc(db, 'things', `${company} - ${id}`));
}
...
export async function getThingsId(target, who) {
...
export async function updateThingsId(target, who, checked, id) {
  const docRef = doc(db, `companies/${target}/${who}`, 'what');
  if (checked) {
    await updateDoc(docRef, {
      thingsId: arrayUnion(id),
    });
  } else {
    await updateDoc(docRef, {
      thingsId: arrayRemove(id),
    });
  }
}

export async function getAllCompaniesForAUser(username) {
  const companiesRef = collection(db, 'companies');
