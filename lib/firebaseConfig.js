...
export async function setUserToCompany(company, username, role = "pending") {
...
export async function getUsersRole(target, username) {
  console.log("22222222:", target, "33333", username);
  const docRef = doc(db, `companies/${target}/${username}`, "what");
  const docSnap = await getDoc(docRef);
  if (docSnap.exists()) {
    console.log("Role data:", docSnap.data().admittedAs);
